...
t('Completion cannot fire while disabled',app.includes("if(t.disabled)return;")&&app.includes("t.classList.contains('finish-study')"));
t('Completion returns one level to block list',app.includes('openStudyDay(unitIndex,dayIndex);')&&!app.includes('openStudySession(unitIndex,dayIndex,sessionIndex+1)'));
t('Completion does not jump to next lesson',app.includes('openStudyDay(unitIndex,dayIndex);')&&!app.includes('openStudyDay(unitIndex+1,0)'));
