...
t('library story entry is active',/id="openHeroesLibrary"[^>]*hero-series-path/.test(html)&&!/id="openHeroesLibrary"[^>]*disabled/.test(html));
t('classic library route is capture-phase persistent',app.includes("openClassicLibrary();return")&&app.includes('},true);'));
t('story library route is capture-phase persistent',/if\(t\.id==='openHeroesLibrary'\)/.test(app)&&/onclick="heroShelf\(this\.dataset\.heroShelf\)"/.test(app));
...
t('current app asset cache busting is 18.8.45',html.includes('app.js?v=18.8.45')&&html.includes('styles.css?v=18.8.45'));
if(unreferenced.length)console.error('Unreferenced button IDs:',unreferenced.join(', '));
console.log(`\nV18.8.45 button/action audit: ${pass}/${pass+fail} passed`);process.exit(fail?1:0);
