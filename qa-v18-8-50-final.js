...
ok('web asset version 18.8.50',/styles\.css\?v=18\.8\.50/.test(html)&&/app\.js\?v=18\.8\.50/.test(html));
ok('service worker registration/cache current',/sw\.js\?v=18\.8\.50/.test(app)&&/ispeak-v18-8-50-release-audit/.test(sw));
ok('server status current',/version:'18\.8\.50'/.test(server));
ok('Android build version is new',/versionCode 5/.test(gradle)&&/versionName '18\.8\.50'/.test(gradle));
...
ok('client does not store teacher password/credentials in localStorage',!/localStorage\.setItem\([^\n]*(teacherPassword|credential|password)/i.test(app));
...
console.log(`FINAL V18.8.50 AUDIT: ${n-fail}/${n} passed`);process.exit(fail?1:0);
