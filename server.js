...
const NATHAN_BETA_PRICE=0.50;
const NATHAN_BETA_PRICE_ENABLED=String(process.env.NATHAN_BETA_PRICE_ENABLED||'true').toLowerCase()!=='false';
const NATHAN_BETA_REVOLUT_URL='https://checkout.revolut.com/pay/046658de-c86d-4b17-b654-ca60fdc910eb';
const TEACHER_LESSON_PAY=8;
const WELCOME_PROMO='welcometoispeakconfidence';
...
function lessonIsTrial(type){return /trial/i.test(String(type||''))}
...
function teacherListedRate(name){return NATHAN_BETA_PRICE_ENABLED&&String(name||'').trim().toLowerCase()==='nathan'?NATHAN_BETA_PRICE:STUDENT_LESSON_PRICE}
function promoAlreadyUsed(email){const e=normEmail(email),db=loadTeachers();return Object.values(db.teachers||{}).some(t=>(t.bookings||[]).some(b=>normEmail(b.email)===e&&b.promoCode===WELCOME_PROMO&&b.paymentStatus==='paid'))}
function bookingQuote(teacher,t,type,email,promoCode){const trial=lessonIsTrial(type),base=teacherListedRate(teacher,t),isNathanBeta=NATHAN_BETA_PRICE_ENABLED&&String(teacher||'').trim().toLowerCase()==='nathan',promo=String(promoCode||'').trim().toLowerCase();let discount=0,promoApplied=false,promoError='';if(promo){if(isNathanBeta)promoError='Promo codes are disabled during Nathan\'s $0.50 beta payment test.';else if(promo!==WELCOME_PROMO)promoError='Promo code is not valid.';else if(!trial)promoError='The welcome offer is only for your first trial lesson.';else if(!email||!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(normEmail(email)))promoError='Enter your student email to use this offer.';else if(promoAlreadyUsed(email))promoError='This welcome offer has already been used.';else{discount=Number((base*.30).toFixed(2));promoApplied=true}}const studentTotal=Number((base-discount).toFixed(2)),teacherEarning=isNathanBeta?0:TEACHER_LESSON_PAY;return {currency:'USD',basePrice:base,discount,studentTotal,promoApplied,promoError,promoCode:promoApplied?WELCOME_PROMO:'',teacherEarning,ispeakGross:Number((studentTotal-teacherEarning).toFixed(2)),platformPromoCost:discount,commissionRate:null,completedTeachingHours:Number(completedTeachingHours(t).toFixed(2)),trial,isNathanBeta}}
...
function analyticsSummary(){const users=loadUsers(),db=loadAnalytics(),events=db.events||[],now=Date.now(),day=86400000,week=7*day;const recent=ms=>events.filter(e=>now-Date.parse(e.at)<=ms),uniq=(rows,k)=>new Set(rows.map(x=>x[k]).filter(Boolean)).size,countBy=(rows,k)=>Object.entries(rows.reduce((a,x)=>{const v=x[k]||'Unknown';a[v]=(a[v]||0)+1;return a},{})).sort((a,b)=>b[1]-a[1]).slice(0,12).map(([name,value])=>({name,value}));const allUsers=Object.values(users.users||{}),new24=allUsers.filter(u=>u.createdAt&&now-Date.parse(u.createdAt)<=day).length,new7=allUsers.filter(u=>u.createdAt&&now-Date.parse(u.createdAt)<=week).length,active24=uniq(recent(day),'session'),active7=uniq(recent(week),'session');return {totalUsers:allUsers.length,newUsers24h:new24,newUsers7d:new7,active24h:active24,active7d:active7,sessions7d:uniq(recent(week),'session'),events7d:recent(week).length,topPages:countBy(recent(week).filter(x=>x.type==='page_view'),'page'),topClicks:countBy(recent(week).filter(x=>x.type==='click'),'target'),languages:countBy(recent(week),'language'),uiLanguages:countBy(recent(week),'uiLanguage'),devices:countBy(recent(week),'device'),sources:countBy(recent(week),'source'),countries:countBy(recent(week),'country'),cities:countBy(recent(week),'city'),funnel:{teacherViews:recent(week).filter(x=>x.type==='teacher_view').length,bookingStarts:recent(week).filter(x=>x.type==='booking_start').length,bookingQuotes:recent(week).filter(x=>x.type==='booking_quote').length,bookings:recent(week).filter(x=>x.type==='booking_complete').length,libraryOpens:recent(week).filter(x=>x.type==='library_open').length,storyOpens:recent(week).filter(x=>x.type==='story_open').length,lessonStarts:recent(week).filter(x=>x.type==='lesson_start').length,ieltsOpens:recent(week).filter(x=>x.type==='ielts_open').length},storage:POSTGRES_ENABLED?'PostgreSQL':'Local JSON fallback'}}
async function handleAPI(req,res,pathname){
 ...
 if(pathname==='/api/account/register'&&req.method==='POST'){
   ...
   if(!validStrongPassword(password))return json(res,400,{error:passwordPolicyText()});
   const db=loadUsers();if(db.users[email])return json(res,409,{error:'An account already exists for this email.'});
   const p=hashPassword(password),token=newToken();recordAnalytics(req,'signup',{user:email});db.users[email]={salt:p.salt,hash:p.hash,tokenHash:crypto.createHash('sha256').update(token).digest('hex'),tokenIssuedAt:new Date().toISOString(),createdAt:new Date().toISOString(),updatedAt:null,state:{}};saveUsers(db);
 ...
 if(pathname==='/api/password-reset/request'&&req.method==='POST'){
   ...
   if(exists){const code=String(crypto.randomInt(100000,1000000)),key=`${kind}:${email}`;PASSWORD_RESETS.set(key,{hash:crypto.createHash('sha256').update(code).digest('hex'),expires:Date.now()+15*60*1000,tries:0});const html=`<div style="font-family:Arial,sans-serif;max-width:560px;margin:auto;padding:28px"><h2 style="color:#0f766e">iSpeak Confidence password reset</h2><p>Your verification code is:</p><div style="font-size:34px;font-weight:900;letter-spacing:8px;color:#102b52">${code}</div><p>This code expires in 15 minutes. If you did not request this, you can ignore this email.</p></div>`;smtpSendMail({to:email,subject:'iSpeak Confidence password reset code',html}).catch(e=>console.warn('[password reset email]',e.message));}
 ...
 if(pathname==='/api/password-reset/confirm'&&req.method==='POST'){
   let body;try{body=JSON.parse(await readBody(req)||'{}')}catch{return json(res,400,{error:'Invalid request.'})}const kind=body.kind==='teacher'?'teacher':'student',email=normEmail(body.email),code=String(body.code||'').trim(),password=String(body.password||'');if(!validStrongPassword(password))return json(res,400,{error:passwordPolicyText()});const key=`${kind}:${email}`,r=PASSWORD_RESETS.get(key);if(!r||r.expires<Date.now()){PASSWORD_RESETS.delete(key);return json(res,400,{error:'Reset code expired. Request a new code.'})}r.tries++;if(r.tries>6){PASSWORD_RESETS.delete(key);return json(res,429,{error:'Too many attempts. Request a new code.'})}if(crypto.createHash('sha256').update(code).digest('hex')!==r.hash)return json(res,400,{error:'Incorrect verification code.'});
   if(kind==='student'){const db=loadUsers(),u=db.users[email];if(!u)return json(res,400,{error:'Account no longer exists.'});const ph=hashPassword(password);u.salt=ph.salt;u.hash=ph.hash;u.tokenHash='';u.updatedAt=new Date().toISOString();saveUsers(db)}else{const db=loadApplications(),a=Object.values(db.applications).find(x=>normEmail(x.email)===email);if(!a)return json(res,400,{error:'Teacher account no longer exists.'});a.passwordHash=hashPassword(password);a.tokenHash='';a.mustChangePassword=false;a.updatedAt=new Date().toISOString();saveApplications(db)}PASSWORD_RESETS.delete(key);return json(res,200,{ok:true});
 }
 ...
 if(pathname==='/api/status'&&req.method==='GET')return json(res,200,{aiConfigured:Boolean(process.env.GEMINI_API_KEY),provider:'Gemini',model:CHAT_MODEL,speechInput:'Gemini audio transcription',tts:TTS_MODEL,version:'V18.7.1',storage:POSTGRES_ENABLED?'PostgreSQL':'JSON fallback',nathanBetaPrice:NATHAN_BETA_PRICE_ENABLED?NATHAN_BETA_PRICE:null,bookingEmailConfigured:Boolean(process.env.SMTP_USER&&process.env.SMTP_APP_PASSWORD),emailConfigured:Boolean(process.env.SMTP_USER&&process.env.SMTP_APP_PASSWORD)});
 ...
 if(pathname==='/api/admin/classrooms'&&req.method==='POST'){
   ...
   if(action==='list'){const rooms=Object.values(cdb.rooms||{}).map(r=>{const found=findBooking(r.bookingId),b=found?.booking||{};return {bookingId:r.bookingId,teacher:r.teacher,studentName:b.name||'',date:b.date||'',time:b.time||'',status:b.status||'',presence:roomPresence(r),recordingReady:recordingReady(r),recordingCount:(r.recordings||[]).length,review:r.review||null,attendance:r.attendance||{}}}).sort((a,b)=>`${b.date}${b.time}`.localeCompare(`${a.date}${a.time}`));return json(res,200,{rooms})}
 ...
 if(pathname==='/api/teacher-application/portal'&&req.method==='POST'){
   ...
   if(body.action==='dashboard'){const open=Object.values(t.availability||{}).reduce((n,a)=>n+(Array.isArray(a)?a.length:0),0);return json(res,200,{teacher,timezone:t.timezone||au.a.timezone||'Asia/Phnom_Penh',availability:t.availability,bookings:t.bookings.slice().sort((a,b)=>`${a.date}${a.time}`.localeCompare(`${b.date}${b.time}`)),openSlotCount:open,payRate,lessonRate,active:t.active!==false,identityVerified:Boolean(au.a.identityVerified),mustChangePassword:Boolean(au.a.mustChangePassword),pendingProfile:au.a.pendingProfile||null,profile:{headline:au.a.headline||'',about:au.a.about||'',specialties:au.a.specialties||[],teachingLanguages:au.a.teachingLanguages||[],photoUrl:au.a.photoUrl||'',videoUrl:au.a.videoUrl||''},stats:stats()})}
   ...
   if(body.action==='rate')return json(res,400,{error:'Lesson pricing is controlled by iSpeak on the server. Nathan may use the temporary $0.50 beta-test price; other tutors remain at the standard rate.'})
 ...
 if(pathname==='/api/teacher-student-messages'&&req.method==='POST'){
   const ta=authApplicant(req);if(!ta||ta.a.status!=='approved')return json(res,401,{error:'Approved teacher login required.'});let body;try{body=JSON.parse(await readBody(req)||'{}')}catch{return json(res,400,{error:'Invalid JSON.'})};const teacher=String(ta.a.publicName||`${ta.a.firstName||''} ${ta.a.lastName||''}`.trim()),tdb=loadTeachers(),t=ensureTeacher(tdb,teacher);if(!t)return json(res,404,{error:'Teacher not found.'});const studentEmail=normEmail(body.studentEmail),bookings=(t.bookings||[]).filter(b=>normEmail(b.email)===studentEmail);if(!bookings.length)return json(res,403,{error:'You can only message students who have booked with you.'});t.studentMessages=t.studentMessages||{};const thread=t.studentMessages[studentEmail]||(t.studentMessages[studentEmail]=[]);if(body.action==='send'){const text=String(body.text||'').trim().slice(0,3000);if(!text)return json(res,400,{error:'Message is empty.'});const check=await aiTeacherConductCheck(text,'student_message');let conduct=null;if(check.flagged)conduct=applyTeacherConductStrike(teacher,check.reason,text,'student_message');if(conduct)conduct={...conduct,ai:check.ai,confidence:check.confidence,category:check.category||''};thread.push({id:'TM-'+crypto.randomBytes(5).toString('hex'),from:'teacher',name:teacher,text,createdAt:new Date().toISOString(),conduct});t.studentMessages[studentEmail]=thread.slice(-300);saveTeachers(tdb);return json(res,200,{ok:true,messages:t.studentMessages[studentEmail],conduct})}return json(res,200,{studentEmail,studentName:bookings[0].name,messages:thread});
 }
 if(pathname==='/api/student-teacher-messages'&&req.method==='POST'){
   const au=authUser(req);if(!au)return json(res,401,{error:'Student login required.'});let body;try{body=JSON.parse(await readBody(req)||'{}')}catch{return json(res,400,{error:'Invalid JSON.'})};const teacher=String(body.teacher||'').trim(),tdb=loadTeachers(),t=tdb.teachers?.[teacher];if(!t||(t.bookings||[]).every(b=>normEmail(b.email)!==au.email))return json(res,403,{error:'You can only message a teacher you have booked with.'});t.studentMessages=t.studentMessages||{};const thread=t.studentMessages[au.email]||(t.studentMessages[au.email]=[]);if(body.action==='send'){const text=String(body.text||'').trim().slice(0,3000);if(!text)return json(res,400,{error:'Message is empty.'});thread.push({id:'SM-'+crypto.randomBytes(5).toString('hex'),from:'student',name:String(au.u?.state?.name||'Student'),text,createdAt:new Date().toISOString()});t.studentMessages[au.email]=thread.slice(-300);saveTeachers(tdb)}return json(res,200,{teacher,messages:t.studentMessages[au.email]});
 }
 ...
 if(pathname==='/api/checkout/quote'&&req.method==='POST'){
   let body;try{body=JSON.parse(await readBody(req)||'{}')}catch{return json(res,400,{error:'Invalid JSON.'})}
   const teacher=String(body.teacher||'').trim(),type=String(body.type||'').trim(),email=normEmail(body.email);const db=loadTeachers(),t=ensureTeacher(db,teacher);if(!t)return json(res,404,{error:'Teacher not found.'});return json(res,200,bookingQuote(teacher,t,type,email,body.promoCode));
 }
 if(pathname==='/api/booking'&&req.method==='POST'){
   ...
   const owner=authUser(req);if(!owner)return json(res,401,{error:'Log in to your student account before paying and booking a lesson.'});if(owner.email!==email)return json(res,403,{error:'The booking email must match your logged-in student account.'});const db=loadTeachers(),t=ensureTeacher(db,teacher);if(!t)return json(res,404,{error:'Teacher not found.'});if(t.active===false)return json(res,409,{error:'This teacher is not currently accepting bookings.'});const check=availableDays(teacher,60),day=check.days.find(x=>x.date===date);if(!day||!day.slots.includes(time))return json(res,409,{error:'That time is no longer available. Please choose another time.'});
   const quote=bookingQuote(teacher,t,type,email,body.promoCode);recordAnalytics(req,'booking_quote',{user:email,target:teacher});if(quote.priceUnavailable)return json(res,409,{error:'This tutor has not published a lesson rate yet. Please choose another tutor or try again later.',quote});if(quote.promoError)return json(res,400,{error:quote.promoError});const testPayment=body.paymentMode==='test'&&process.env.NODE_ENV!=='production';const revolutBeta=body.paymentMode==='revolut_external'&&quote.isNathanBeta&&quote.studentTotal===NATHAN_BETA_PRICE;if(!testPayment&&!revolutBeta)return json(res,402,{error:'Secure card payments are not connected yet for this tutor.',requiresPaymentProvider:true,quote});const ownerEmail=owner.email;const bookingId='BK-'+crypto.randomBytes(4).toString('hex').toUpperCase(),safe={id:bookingId,teacher,teacherApplicationId:t.applicationId||'',subject,name,email,date,time,type,message,ownerEmail,status:revolutBeta?'payment_pending':'confirmed',paymentStatus:revolutBeta?'pending_verification':'paid',paymentMode:revolutBeta?'revolut_external':'test',basePrice:quote.basePrice,discount:quote.discount,amountPaid:revolutBeta?0:quote.studentTotal,amountDue:quote.studentTotal,promoCode:quote.promoCode,teacherEarning:quote.teacherEarning,earningAmount:quote.teacherEarning,commissionRate:quote.commissionRate,durationHours:lessonDurationHours(type),created:new Date().toISOString()};t.bookings=t.bookings||[];t.bookings.push(safe);saveTeachers(db);recordAnalytics(req,'booking_complete',{user:email,target:teacher});
   ...
   return json(res,200,{ok:true,bookingId,status:safe.status,paymentStatus:safe.paymentStatus,quote,testPayment,checkoutUrl:revolutBeta?NATHAN_BETA_REVOLUT_URL:null,requiresManualVerification:revolutBeta})
 }
...
function listenOn(port){
 activePort=port;
 server.listen(port,()=>{console.log(`\niSpeak Confidence V18.7.1: http://localhost:${port}`);console.log(process.env.GEMINI_API_KEY?`Gemini Smart AI: READY (${CHAT_MODEL})`:'Gemini Smart AI: OFF — add GEMINI_API_KEY to .env');console.log(process.env.GEMINI_API_KEY&&AI_CONDUCT_ENABLED?'Classroom AI conduct monitoring: READY':'Classroom AI conduct monitoring: FALLBACK RULES ONLY');console.log(process.env.SMTP_USER&&process.env.SMTP_APP_PASSWORD?'Certificate Email: READY':'Certificate Email: OFF — add SMTP_USER and SMTP_APP_PASSWORD to .env');console.log('Press Ctrl+C to stop.\n')});
}
