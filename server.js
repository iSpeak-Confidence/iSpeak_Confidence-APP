...
function emailConfigured(){return Boolean(process.env.SMTP_USER&&(process.env.SMTP_APP_PASSWORD||process.env.SMTP_PASSWORD))}
function smtpSendMail({to,subject,html}){
 const host=process.env.SMTP_HOST||'smtp.gmail.com',port=Number(process.env.SMTP_PORT||465),user=process.env.SMTP_USER,pass=process.env.SMTP_APP_PASSWORD||process.env.SMTP_PASSWORD,fromName=process.env.CERT_FROM_NAME||'iSpeak Confidence';
...
async function handleAPI(req,res,pathname){
 ...
 if(pathname==='/api/account/login'&&req.method==='POST'){
   ...
   if(deviceId&&!u.trustedDevices.includes(deviceId)){if(!emailReady)return json(res,503,{error:'Email verification is not configured on this server. Add SMTP_USER and SMTP_APP_PASSWORD in Render Environment, then redeploy.'});const code=String(crypto.randomInt(100000,1000000)),key=`student:${email}:${deviceId}`;DEVICE_VERIFICATIONS.set(key,{hash:crypto.createHash('sha256').update(code).digest('hex'),expires:Date.now()+10*60*1000,tries:0});try{await smtpSendMail({to:email,subject:'iSpeak Confidence new-device verification',html:`<div style="font-family:Arial,sans-serif;max-width:560px;margin:auto;padding:28px"><h2 style="color:#0f766e">New device sign-in</h2><p>Use this verification code to finish signing in:</p><div style="font-size:34px;font-weight:900;letter-spacing:8px;color:#102b52">${code}</div><p>This code expires in 10 minutes.</p></div>`})}catch(e){DEVICE_VERIFICATIONS.delete(key);console.error('[device verification email]',e.message);return json(res,503,{error:'We could not send your verification email. Check the server email settings and try again.'})}saveUsers(db);return json(res,200,{verificationRequired:true,email});}
 ...
 if(pathname==='/api/password-reset/request'&&req.method==='POST'){
   console.log('[password reset request] request received');
   ...
   if(exists){
     if(!emailConfigured())return json(res,503,{error:'Password reset email is not configured on this server. Add SMTP_USER and SMTP_APP_PASSWORD in Render Environment.'});
 ...
 if(pathname==='/api/status'&&req.method==='GET')return json(res,200,{aiConfigured:Boolean(process.env.GEMINI_API_KEY),provider:'Gemini',model:CHAT_MODEL,speechInput:'Gemini audio transcription',tts:TTS_MODEL,storage:POSTGRES_ENABLED?'PostgreSQL':'JSON fallback',nathanBetaPrice:false?STUDENT_LESSON_PRICE:null,bookingEmailConfigured:emailConfigured(),emailConfigured:emailConfigured(),emailHost:process.env.SMTP_HOST||'smtp.gmail.com',emailPort:Number(process.env.SMTP_PORT||465)});
 ...
 if(pathname==='/api/teacher-application/login'&&req.method==='POST'){
   let body;try{body=JSON.parse(await readBody(req,1_000_000)||'{}')}catch{return json(res,400,{error:'Invalid JSON.'})}const email=normEmail(body.email),password=String(body.password||''),deviceId=String(body.deviceId||'').trim().slice(0,180),db=loadApplications();const entry=Object.entries(db.applications).find(([,a])=>a.email===email);if(!entry||!verifyPassword(password,entry[1].passwordHash))return json(res,403,{error:'Teacher email or password is incorrect. Check the exact email shown in Admin, or reset the teacher password from Admin.'});const [id,a]=entry;a.trustedDevices=Array.isArray(a.trustedDevices)?a.trustedDevices:[];const emailReady=emailConfigured();if(deviceId&&!a.trustedDevices.includes(deviceId)){if(!emailReady)return json(res,503,{error:'Email verification is not configured on this server. Add SMTP_USER and SMTP_APP_PASSWORD in Render Environment, then redeploy.'});const code=String(crypto.randomInt(100000,1000000)),key=`teacher:${email}:${deviceId}`;DEVICE_VERIFICATIONS.set(key,{hash:crypto.createHash('sha256').update(code).digest('hex'),expires:Date.now()+10*60*1000,tries:0});try{await smtpSendMail({to:email,subject:'iSpeak Confidence teacher new-device verification',html:`<div style="font-family:Arial,sans-serif;max-width:560px;margin:auto;padding:28px"><h2 style="color:#0f766e">Teacher Portal new-device sign-in</h2><p>Use this verification code to finish signing in:</p><div style="font-size:34px;font-weight:900;letter-spacing:8px;color:#102b52">${code}</div><p>This code expires in 10 minutes.</p></div>`})}catch(e){DEVICE_VERIFICATIONS.delete(key);console.error('[teacher device verification email]',e.message);return json(res,503,{error:'We could not send your verification email. Check the server email settings and try again.'})}saveApplications(db);return json(res,200,{verificationRequired:true,email,application:applicationPublic(a)});}if(deviceId&&!a.trustedDevices.includes(deviceId)){a.trustedDevices.push(deviceId);a.trustedDevices=a.trustedDevices.slice(-12)}const token=newToken();a.tokenHash=crypto.createHash('sha256').update(token).digest('hex');a.tokenIssuedAt=new Date().toISOString();saveApplications(db);return json(res,200,{ok:true,token,application:applicationPublic(a)})
 }
...
function listenOn(port){
 activePort=port;
 server.listen(port,()=>{console.log(`\niSpeak Confidence: http://localhost:${port}`);console.log('Revolut payment link: READY — single fixed $13 checkout link');console.log(process.env.GEMINI_API_KEY?`Gemini Smart AI: READY (${CHAT_MODEL})`:'Gemini Smart AI: OFF — add GEMINI_API_KEY to .env');console.log(process.env.GEMINI_API_KEY&&AI_CONDUCT_ENABLED?'Classroom AI conduct monitoring: READY':'Classroom AI conduct monitoring: FALLBACK RULES ONLY');console.log(emailConfigured()?'Account & Certificate Email: READY':'Account & Certificate Email: OFF — add SMTP_USER and SMTP_APP_PASSWORD to .env');console.log('Press Ctrl+C to stop.\n')});
}
